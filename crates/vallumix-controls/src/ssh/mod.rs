...
macro_rules! def_sshd_control {
    ($name:ident, $id:expr, $desc:expr, $sev:expr, $directive:expr, $expected:expr, $apply:expr) => {
        ...
        impl $name {
            pub fn new() -> Self {
                $name(super::common::SshdConfigControl::new(
                    $id, $desc, $sev, $directive, $expected, $apply,
                ))
            }
            pub fn with_path(sshd_config_path: std::path::PathBuf) -> Self {
                $name(super::common::SshdConfigControl::with_path(
                    $id,
                    $desc,
                    $sev,
                    sshd_config_path,
...
pub mod ssh_disable_empty_passwords {
    def_sshd_control!(
        SshDisableEmptyPasswords,
        "5.2.3",
        "Ensure SSH empty passwords are disabled",
        vallumix_core::control::Severity::High,
...
pub mod ssh_set_crypto_policy {
    def_sshd_control!(
        SshSetCryptoPolicy,
        "5.2.11",
        "Ensure SSH crypto policy is configured",
        vallumix_core::control::Severity::Medium,
        "Ciphers",
        vec![],
