    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        if has_allowusers || has_allowgroups {
            Ok(CheckResult {
                ...
                evidence: "AllowUsers or AllowGroups configured".into(),
                message: None,
            })
        } else {
            Ok(CheckResult {
                status: CheckStatus::NonCompliant,
                evidence: "Neither AllowUsers nor AllowGroups configured".into(),
                message: Some("Configure AllowUsers or AllowGroups".into()),
            })
        }
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would configure AllowUsers".into()),
            });
        }
        let content = fs::read_to_string(&self.sshd_config_path).unwrap_or_default();
        ...
        Ok(ApplyResult {
            ...
            backup_path: None,
            message: Some("added AllowUsers".into()),
        })
    }
