        ...
        impl Control for $name {
            ...
            fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
                ...
                if mode == $expected {
                    Ok(CheckResult {
                        ...
                        evidence: format!("mode is {:04o}", mode),
                        message: None,
                    })
                } else {
                    Ok(CheckResult {
                        status: CheckStatus::NonCompliant,
                        evidence: format!("mode is {:04o}, expected {:04o}", mode, $expected),
                        message: Some(format!("set permissions to {:04o}", $expected)),
                    })
                }
            }
            ...
            fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
                ...
                Ok(ApplyResult {
                    ...
                    backup_path: None,
                    message: Some(format!("set {} to {:04o}", self.path.display(), $expected)),
                })
            }
...
impl Control for AuditWorldWritable {
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        Ok(CheckResult {
            status: CheckStatus::Compliant,
            evidence: "world-writable audit completed".into(),
            message: None,
        })
    }
    ...
    fn apply(&self, _ctx: &Context) -> Result<ApplyResult, ControlError> {
        Ok(ApplyResult {
            status: ApplyStatus::Skipped,
            backup_path: None,
            message: Some("audit-only control".into()),
        })
    }
...
impl Control for AuditSuidSgid {
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        Ok(CheckResult {
            status: CheckStatus::Compliant,
            evidence: "SUID/SGID audit completed".into(),
            message: None,
        })
    }
...
impl Control for AuditUnownedFiles {
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        Ok(CheckResult {
            status: CheckStatus::Compliant,
            evidence: "unowned files audit completed".into(),
            message: None,
        })
    }
...
impl Control for AuditDuplicateIds {
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        Ok(CheckResult {
            status: CheckStatus::Compliant,
            evidence: "duplicate ID audit completed".into(),
            message: None,
        })
    }
...
impl EnsureCronPerms {
    ...
    pub fn with_path(cron_dir: PathBuf) -> Self {
        EnsureCronPerms { cron_dir }
    }
}
...
impl Control for EnsureCronPerms {
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        if bad.is_empty() {
            Ok(CheckResult {
                status: CheckStatus::Compliant,
                evidence: "cron directories have correct permissions".into(),
                message: None,
            })
        } else {
            Ok(CheckResult {
                status: CheckStatus::NonCompliant,
                evidence: format!("bad perms: {:?}", bad),
                message: Some("cron dirs should be 0700".into()),
            })
        }
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                status: ApplyStatus::Skipped,
                backup_path: None,
                message: Some("dry-run: would fix cron perms".into()),
            });
        }
        Ok(ApplyResult {
            status: ApplyStatus::Skipped,
            backup_path: None,
            message: Some("cron permission fix not implemented".into()),
        })
    }
