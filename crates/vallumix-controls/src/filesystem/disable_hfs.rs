    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        Ok(if present {
            CheckResult {
                ...
                evidence: "hfs found in /proc/filesystems".into(),
                message: Some("hfs is available".into()),
            }
        } else {
            CheckResult {
                status: CheckStatus::Compliant,
                evidence: "hfs not found in /proc/filesystems".into(),
                message: None,
            }
        })
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would disable hfs".into()),
            });
        }
...
mod tests {
    ...
    fn apply_writes_modprobe_file() {
        ...
        let ctrl =
            DisableHfs::with_paths(tmpdir.path().join("fs"), tmpdir.path().join("modprobe.d"));
        let result = ctrl.apply(&ctx()).unwrap();
        assert_eq!(result.status, ApplyStatus::Applied);
        let content =
            fs::read_to_string(tmpdir.path().join("modprobe.d/vallumix-disable-hfs.conf")).unwrap();
