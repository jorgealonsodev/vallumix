    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        Ok(if present {
            CheckResult {
                ...
                evidence: "usb-storage found in /proc/filesystems".into(),
                message: Some("usb-storage is available".into()),
            }
        } else {
            CheckResult {
                status: CheckStatus::Compliant,
                evidence: "usb-storage not found in /proc/filesystems".into(),
                message: None,
            }
        })
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would disable usb-storage".into()),
            });
        }
...
mod tests {
    ...
    fn check_compliant_when_absent() {
        ...
        let ctrl =
            DisableUsbStorage::with_paths(tmp.path().into(), tmp.path().parent().unwrap().into());
    ...
    fn apply_writes_modprobe_file() {
        ...
        let ctrl = DisableUsbStorage::with_paths(
            tmpdir.path().join("fs"),
        ...
        let content = fs::read_to_string(
            tmpdir
                .path()
                .join("modprobe.d/vallumix-disable-usb-storage.conf"),
        )
        .unwrap();
