    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        Ok(if present {
            CheckResult {
                ...
                evidence: "jffs2 found in /proc/filesystems".into(),
                message: Some("jffs2 is available".into()),
            }
        } else {
            CheckResult {
                status: CheckStatus::Compliant,
                evidence: "jffs2 not found in /proc/filesystems".into(),
                message: None,
            }
        })
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would disable jffs2".into()),
            });
        }
...
mod tests {
    ...
    fn apply_writes_modprobe_file() {
        ...
        let ctrl =
            DisableJffs2::with_paths(tmpdir.path().join("fs"), tmpdir.path().join("modprobe.d"));
        let result = ctrl.apply(&ctx()).unwrap();
        assert_eq!(result.status, ApplyStatus::Applied);
        let content =
            fs::read_to_string(tmpdir.path().join("modprobe.d/vallumix-disable-jffs2.conf"))
                .unwrap();
