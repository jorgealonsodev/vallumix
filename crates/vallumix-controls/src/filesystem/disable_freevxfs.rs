...
#[derive(Debug, Clone)]
pub struct DisableFreevxfs {
    filesystems_path: PathBuf,
    modprobe_dir: PathBuf,
}

impl Default for DisableFreevxfs {
    fn default() -> Self {
        DisableFreevxfs {
...
impl DisableFreevxfs {
    pub fn new() -> Self { Self::default() }
    pub fn with_paths(filesystems_path: PathBuf, modprobe_dir: PathBuf) -> Self {
        DisableFreevxfs { filesystems_path, modprobe_dir }
    }
}

impl Control for DisableFreevxfs {
    fn id(&self) -> &str { "1.1.1.2" }
    fn description(&self) -> &str { "Ensure mounting of freevxfs filesystems is disabled" }
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        let present = content.lines().any(|line| line.contains("freevxfs"));
        Ok(if present {
            CheckResult { status: CheckStatus::NonCompliant, evidence: "freevxfs found in /proc/filesystems".into(), message: Some("freevxfs is available".into()) }
        } else {
            CheckResult { status: CheckStatus::Compliant, evidence: "freevxfs not found in /proc/filesystems".into(), message: None }
        })
    }

    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult { status: ApplyStatus::Skipped, backup_path: None, message: Some("dry-run: would disable freevxfs".into()) });
        }
        let file = self.modprobe_dir.join("vallumix-disable-freevxfs.conf");
...
mod tests {
    ...
    fn check_compliant_when_absent() {
        ...
        let ctrl = DisableFreevxfs::with_paths(tmp.path().into(), tmp.path().parent().unwrap().into());
    ...
    fn check_non_compliant_when_present() {
        let mut tmp = tempfile::NamedTempFile::new().unwrap();
        writeln!(tmp, "nodev\tfreevxfs").unwrap();
        let ctrl = DisableFreevxfs::with_paths(tmp.path().into(), tmp.path().parent().unwrap().into());
    ...
    fn apply_writes_modprobe_file() {
        ...
        let ctrl = DisableFreevxfs::with_paths(tmpdir.path().join("fs"), tmpdir.path().join("modprobe.d"));
        let result = ctrl.apply(&ctx()).unwrap();
        assert_eq!(result.status, ApplyStatus::Applied);
        let content = fs::read_to_string(tmpdir.path().join("modprobe.d/vallumix-disable-freevxfs.conf")).unwrap();
        assert!(content.contains("install freevxfs /bin/true"));
    }
}
