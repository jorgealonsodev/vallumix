    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        Ok(if present {
            CheckResult {
                ...
                evidence: "squashfs found in /proc/filesystems".into(),
                message: Some("squashfs is available".into()),
            }
        } else {
            CheckResult {
                status: CheckStatus::Compliant,
                evidence: "squashfs not found in /proc/filesystems".into(),
                message: None,
            }
        })
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would disable squashfs".into()),
            });
        }
...
mod tests {
    ...
    fn check_compliant_when_absent() {
        ...
        let ctrl =
            DisableSquashfs::with_paths(tmp.path().into(), tmp.path().parent().unwrap().into());
    ...
    fn apply_writes_modprobe_file() {
        ...
        let ctrl =
            DisableSquashfs::with_paths(tmpdir.path().join("fs"), tmpdir.path().join("modprobe.d"));
        let result = ctrl.apply(&ctx()).unwrap();
        assert_eq!(result.status, ApplyStatus::Applied);
        let content = fs::read_to_string(
            tmpdir
                .path()
                .join("modprobe.d/vallumix-disable-squashfs.conf"),
        )
        .unwrap();
