...
mod tests {
    ...
    fn check_compliant_when_ip_forward_0() {
        ...
        let ctrl =
            SysctlIpForwarding::with_paths(tmp.path().into(), tmp.path().parent().unwrap().into());
    ...
    fn apply_writes_sysctl_dropin() {
        ...
        let ctrl = SysctlIpForwarding::with_paths(
            tmpdir.path().join("ip_forward"),
            tmpdir.path().join("sysctl.d"),
        );
        let ctx = Context::with_paths(
            "test".into(),
            Distro::Debian12,
        ...
        let content =
            fs::read_to_string(tmpdir.path().join("sysctl.d/99-vallumix-ip-forward.conf")).unwrap();
