...
impl ConfigureFirewalld {
    ...
    pub fn with_path(systemctl_path: PathBuf) -> Self {
        ConfigureFirewalld { systemctl_path }
    }
}
    ...
    fn check(&self, _ctx: &Context) -> Result<CheckResult, ControlError> {
        ...
        if active {
            Ok(CheckResult {
                ...
                evidence: "firewalld is active".into(),
                message: None,
            })
        } else {
            ...
            if nft_active {
                Ok(CheckResult {
                    status: CheckStatus::Compliant,
                    evidence: "nftables is active".into(),
                    message: None,
                })
            } else {
                Ok(CheckResult {
                    status: CheckStatus::NonCompliant,
                    evidence: "no firewall daemon active".into(),
                    message: Some("install and enable firewalld or nftables".into()),
                })
            }
        }
    }
    ...
    fn apply(&self, ctx: &Context) -> Result<ApplyResult, ControlError> {
        if ctx.dry_run {
            return Ok(ApplyResult {
                ...
                backup_path: None,
                message: Some("dry-run: would enable firewall".into()),
            });
        }
        Ok(ApplyResult {
            status: ApplyStatus::Skipped,
            backup_path: None,
            message: Some("firewall installation not implemented".into()),
        })
    }
