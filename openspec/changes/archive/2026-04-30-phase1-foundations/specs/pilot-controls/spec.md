...
The `disable_cramfs` control MUST implement the `Control` trait with `id()` returning `"1.1.1.1"`, `severity()` returning `Severity::Low`, and `applicable_distros()` returning all supported distros. `check()` MUST verify the `cramfs` filesystem type is not loaded (via `/proc/filesystems` or `modprobe --show`). `apply()` MUST add `install cramfs /bin/true` to `/etc/modprobe.d/` config. `rollback()` MUST remove the config entry.
...
The `disable_avahi` control MUST implement the `Control` trait with `id()` returning a CIS 2.x ID, `severity()` returning `Severity::Medium`. `check()` MUST verify the Avahi daemon service is disabled and stopped. `apply()` MUST disable and stop the service using the distro-appropriate service manager. `rollback()` MUST re-enable the service.
...
The `sysctl_ip_forwarding` control MUST implement `Control` with `id()` returning a CIS 3.x ID, `severity()` returning `Severity::Medium`. `check()` MUST read `/proc/sys/net/ipv4/ip_forward` and verify value is `0`. `apply()` MUST set `net.ipv4.ip_forward = 0` in a sysctl drop-in file AND via the running kernel. `rollback()` MUST restore the previous value.
...
The `ensure_perms_passwd` control MUST implement `Control` with `id()` returning a CIS 6.x ID, `severity()` returning `Severity::High`. `check()` MUST verify that `/etc/passwd` has permissions `0644` (owner rw, group r, others r). `apply()` MUST set permissions to `0644`. `rollback()` MUST restore original permissions from backup.
