...
(Previously: `disable_cramfs` did not implement `category()` — the `Control` trait had no `category` method.)
...
(Previously: `disable_avahi` did not implement `category()`.)
...
(Previously: `sysctl_ip_forwarding` did not implement `category()`.)
...
(Previously: `ssh_disable_root_login` did not implement `category()`.)
...
The `ensure_perms_passwd` control MUST implement `Control` with `id()` returning a CIS 6.x ID, `severity()` returning `Severity::High`, and `category()` returning `Category::Maintenance`. `check()` MUST verify that `/etc/passwd` has permissions `0644`. `apply()` MUST set permissions to `0644`. `rollback()` MUST restore original permissions from backup.
(Previously: `ensure_perms_passwd` did not implement `category()`.)
